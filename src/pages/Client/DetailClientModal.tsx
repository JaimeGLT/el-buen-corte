...
export const DetailClientModal = ({ setModalState, id }: ModalServiceProps) => {

    const { data: client, loading } = getHook<ClientDetailType>(`/client/${id}`);
                    ...
                    <>
                        <div className="flex flex-col text-paragraph">
                            ...
                            <div className="flex flex-col gap-1">
                                <p className="flex text-sm items-center gap-2"><Mail size={15}/> {client?.email ? client.email : "---"}</p>
                        ...
                        <div className="border border-border-input rounded-xl p-5 flex flex-col gap-5 mt-5">
                            ...
                            <p className="text-sm text-paragraph">{client?.observations ? client.observations : "No hay preferencias ni observaciones"}</p>
                        </div>
                        ...
                        <div className="mt-5 border border-border-input rounded-xl p-5">
                            ...
                            {
                                !client?.citas.length ? <span className="text-sm text-paragraph">No hay historial de citas</span> :  
